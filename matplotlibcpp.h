...
#  endif
        ...
        struct Keyword
        {
            ...
            std::optional<std::wstring> wstring_value;

            Keyword(std::string key, double value)
            : key(std::move(key)), double_value(value)
            {}

            Keyword(std::string key, const std::string &value)
            : key(std::move(key)), string_value(value)
            {}

            Keyword(std::string key, const std::wstring & value)
            : key(key), wstring_value(value)
            {}
        };

        PyObject * ConstructKeywordArgs(const std::vector<Keyword> & keywords)
        {
            ...
            for(auto & i : keywords)
            {
                if(i.double_value)
                {
                    PyDict_SetItemString(kwargs, i.key.data(), PyFloat_FromDouble(i.double_value.value()));
                }
                else if(i.string_value)
                {
                    PyDict_SetItemString(kwargs, i.key.data(), PyString_FromString(i.string_value.value().data()));
                }
                else
                {
                    PyDict_SetItemString(kwargs, i.key.data(), PyUnicode_FromString((char*)i.wstring_value.value().data()));
                }
            }
            return kwargs;
        }
...
// must be called before the first regular call to matplotlib to have any effect
    ...
    bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y,
              const std::vector<detail::Keyword> &keywords)
    {
...
// a non-numpy alternative for `get_2darray()`.
    ...
    inline bool subplots_adjust(const std::vector<detail::Keyword> &keywords = {})
    {
    ...
    bool quiver(const std::vector<NumericX> &x, const std::vector<NumericY> &y, const std::vector<NumericU> &u,
                const std::vector<NumericW> &w, const std::vector<detail::Keyword> &keywords = {})
    {
    ...
    template<typename Numeric>
    bool plot(const std::vector<Numeric> &y, const std::vector<detail::Keyword> &keywords)
    {
        std::vector<Numeric> x(y.size());
        for (size_t i = 0; i < x.size(); ++i) x.at(i) = i;
    ...
    template<typename Numeric>
    inline void xticks(const std::vector<Numeric> &ticks, const std::vector<detail::Keyword> &keywords)
    {
    ...
    template<typename Numeric>
    inline void yticks(const std::vector<Numeric> &ticks, const std::vector<detail::Keyword> &keywords)
    {
        yticks(ticks, {}, keywords);
    }

    inline void tick_params(const std::vector<detail::Keyword> &keywords, const std::string axis = "both")
    {
        // construct positional args
        PyObject *args;
        args = PyTuple_New(1);
    ...
    inline void title(const std::string &titlestr, const std::vector<detail::Keyword> &keywords = {})
    {
    ...
    inline void suptitle(const std::string &suptitlestr, const std::vector<detail::Keyword> &keywords = {})
    {
    ...
    inline void xlabel(const std::string &str, const std::vector<detail::Keyword> &keywords = {})
    {
    ...
    inline void ylabel(const std::string &str, const std::vector<detail::Keyword> &keywords = {})
    {
        PyObject *pystr = PyString_FromString(str.c_str());
        PyObject *args = PyTuple_New(1);
        PyTuple_SetItem(args, 0, pystr);
    ...
    ginput(const int numClicks = 1, const std::vector<detail::Keyword> &keywords = {})
    {
        PyObject *args = PyTuple_New(1);
 ...
 */
    ...
    inline bool
    plot(const std::vector<double> &x, const std::vector<double> &y, const std::vector<detail::Keyword> &keywords)
    {
